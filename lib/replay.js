...
module.exports = function(dir, prefetch) {
  var records = fs.readdirSync(dir).filter(function(filename) {
  ...
  }).sort().map(function(metadata_filename) {
    ...
    if (prefetch) {
      ...
      if (record.response.data) {
        ...
        record.response.headers['transfer-encoding'] = 'chunked'
  ...
  function middleware(req, res, next) {
    ...
    var matching_records = records.filter(function(record) {
      return matcher(record.request)
    })
    var record

    // Use the first non-played record
    for (var i = 0; i < matching_records.length; i++) if (matching_records[i].played === 0) {
      record = matching_records[i]
      break
    }

    // Or use the first if all is played already
    if (!record) record = matching_records[0]
